...
const CIP_SESSIONS =
  'Du 21 sept. 2026 au 23 avr. 2027 (candidatures dès le 3 févr. 2026) · Du 22 mars au 22 oct. 2027 (candidatures dès le 3 oct. 2026)'
...
export function getCertifianteRappelHref(briefId) {
  const geo = CERTIFIANTES_GEO_BY_BRIEF[briefId]
  if (!geo) return '/contact?sujet=information&message=Je+souhaite+%C3%AAtre+rappel%C3%A9'
  const params = new URLSearchParams()
  params.set('sujet', geo.sujetContact)
  params.set('message', `Je souhaite être rappelé au sujet de ${geo.sujetLabel}.`)
  appendUtm(params, geo)
  return `/contact?${params.toString()}`
}
...
export function getCertifianteReunionHref(briefId) {
  const geo = CERTIFIANTES_GEO_BY_BRIEF[briefId]
  if (!geo?.formationKey) return '/s-inscrire'
  return `/s-inscrire?formation=${geo.formationKey}`
}
